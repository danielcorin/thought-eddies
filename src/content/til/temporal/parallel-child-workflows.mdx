...
In this example, we use `dataclass`es as the inputs and outputs to all activities and workflows to allow us to evolve the inputs and outputs without breaking the data contract between the <Sidenote anchor="workflow components" content="For example, if an activity returned `str`, we would struggle to add an additional return parameter without changing the return type. We can get in front of this issue by always returning classes." />.
