...
Here are the <Sidenote anchor="results" content="Extracted by pasting an image of the table from the notebook into a Cursor/Sonnet prompt" /> from training the `resnet50` model:
...
I <Sidenote anchor="generated another 75 examples" content="Visual inspection of the labeled images in the `Datablock` didn't reveal any obvious difference between the two voices, so I was relying on some model magic to happen with the finetuning" /> per voice then trained `resnet34` again.
