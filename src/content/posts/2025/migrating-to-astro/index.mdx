...
I'd heard about Astro and wanted more put a more personal touch on my web presence.

I wrote an article here or there and then one day, after I'd run into a bit of a block working on [Tomo](https://wvlen.llc/apps/tomo), I had the itch to build Thought Eddies out.

I have several post types I write frequently, so I needed to build sections for that.
Then I added support for a bunch of the [shortcodes](https://github.com/danielcorin/blog/tree/main/layouts/shortcodes), I'd built over the years in Hugo.
...
Each too a few 100ms to build each.
I tried switch to a different library, started taking more than one second each.
...
Hugo claims to be fast and we talk about how the Javascript web can be slow and bloated, but was it really this bad?
How had I never run into this before?
...
At build time, a webserver starts and a process requests the content, the persists it as a static HTML file to be served in production.

No API calls.

What was taking so long?

I pulled it down and started to low around
...
_By no means_ fast or reasonable all you web purist will tell me because the web is HTML, CSS, and vanilla Javascript and everything should be 10kb and load in milliseconds.
