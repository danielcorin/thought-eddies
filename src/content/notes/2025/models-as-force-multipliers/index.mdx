...
updatedAt: 2025-04-08T12:29:01-04:00
...
It's still surprising how much code we can write with simple instructions and how much progress we can make on a project in fixed time relative to what has previous been possible.
But many of the realities of building software that needs to run reliably and be secure have not changed.
If the engineer is still the responsible party for handling and being accountable for these challenges, then models are simply tools.

It would be ill advised to generate and merge code for a system you are oncall for without carefully reading and testing that code yourself.
