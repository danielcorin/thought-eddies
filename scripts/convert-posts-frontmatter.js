...
async function convertPost(filePath) {
  ...
  const newFrontmatter = {
    ...
    updatedAt: parseDate(
      frontmatter.updatedAt || frontmatter.date || frontmatter.createdAt
    ),
    publishedAt:
      frontmatter.draft === false
        ? parseDate(frontmatter.date || frontmatter.publishedAt)
        : undefined,
...
function parseDate(dateStr) {
  ...
  if (isNaN(date.getTime())) {
    console.warn(
      `${colors.yellow}Warning: Invalid date "${dateStr}"${colors.reset}`
    );
    return new Date();
  }
...
async function main() {
  ...
  for (const file of files) {
    try {
      ...
      const hasChanges =
        JSON.stringify(result.oldFrontmatter) !==
        JSON.stringify(result.newFrontmatter);

      if (!hasChanges) {
        if (verbose) {
          console.log(
            `${colors.yellow}SKIP${colors.reset} ${file} (no changes needed)`
          );
        }
        skipped++;
        continue;
      ...
      if (verbose || !dryRun) {
        ...
        if (verbose) {
          ...
          console.log(
            '    ' +
              JSON.stringify(result.oldFrontmatter, null, 2)
                .split('\n')
                .join('\n    ')
          );
          ...
          console.log(
            '    ' +
              JSON.stringify(result.newFrontmatter, null, 2)
                .split('\n')
                .join('\n    ')
          );
        }
      ...
      console.error(
        `${colors.red}ERROR${colors.reset} ${file}: ${error.message}`
      );
      errors++;
    }
  }
